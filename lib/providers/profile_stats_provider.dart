...
final duelStatsProvider = FutureProvider<DuelStats?>((ref) async {
  final userId = ref.watch(currentUserIdProvider);
  if (userId == null) return null;
  return ref.read(profileStatsServiceProvider).getDuelStats(userId);
});

final totalRepsProvider = FutureProvider<int>((ref) async {
  final userId = ref.watch(currentUserIdProvider);
  if (userId == null) return 0;
  return ref.read(profileStatsServiceProvider).getTotalReps(userId);
});

final recentDuelsProvider = FutureProvider<List<RecentDuelItem>>((ref) async {
  final userId = ref.watch(currentUserIdProvider);
  if (userId == null) return [];
  return ref.read(profileStatsServiceProvider).getRecentDuels(userId);
});

final achievementsProvider = FutureProvider<List<AchievementInfo>>((ref) async {
  final userId = ref.watch(currentUserIdProvider);
  if (userId == null) return [];
